document.addEventListener('DOMContentLoaded', function() {
  ...
  if (selectedPlan) {
    const selectedRadio = document.querySelector(`input[name="tier"][value="${selectedPlan}"]`);
    if (selectedRadio) {
  ...
  form.addEventListener('submit', async function(event) {
    ...
    const tierSelected = Array.from(tierRadios).find(radio => radio.checked);
    ...
    const payload = {
      ...
      tier: tierSelected ? tierSelected.value : '',
    ...
    try {
      ...
      if (!response.ok) {
        const message = result && result.error ? result.error : 'Registration failed.';
      ...
      showErrorMessage('Unable to reach the server. Please try again.');
    }
  });
});
